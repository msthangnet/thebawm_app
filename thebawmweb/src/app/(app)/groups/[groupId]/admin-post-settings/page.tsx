...
import { PostPermissions, DefaultPostPermissions, UserTypes, GroupInfo } from "@/lib/types";
...
export default function GroupAdminPostSettings() {
    const { user, userProfile, loading: authLoading } = useAuth();
    const router = useRouter();
    const params = useParams();
    const groupIdSlug = params.groupId as string;
    ...
    const fetchGroupAndPermissions = useCallback(async () => {
        if (!groupIdSlug || !user) return;
        setLoading(true);
        try {
            // 1. Fetch Group Info by slug
            const groupsRef = collection(db, "groups");
            const q = query(groupsRef, where("groupId", "==", groupIdSlug));
            ...
            const groupDocSnap = groupQuerySnapshot.docs[0];
            const groupData = { ...groupDocSnap.data(), id: groupDocSnap.id } as GroupInfo;
            ...
            if (!isOwner && !isSiteAdmin) {
                toast({ title: "Access Denied", description: "You don't have permission to manage this group's settings.", variant: "destructive" });
                router.replace(`/groups/${groupIdSlug}`);
                return;
            }
            
            // 3. Fetch Group-specific or Default Permissions
            const settingsRef = doc(db, "groups", groupData.id, "settings", "postPermissions");
    ...
    }, [groupIdSlug, user, userProfile, router, toast]);
    ...
    }, [authLoading, user, fetchGroupAndPermissions, router]);
    ...
    const handleSave = async () => {
        if (!permissions || !groupInfo) return;
        setSaving(true);
        try {
            const settingsRef = doc(db, "groups", groupInfo.id, "settings", "postPermissions");
            await setDoc(settingsRef, permissions, { merge: true });
            toast({ title: "Settings Saved", description: "Group post permissions have been updated." });
        } catch (error) {
    ...
    if (!permissions || !groupInfo) {
        return (
            <div className="flex h-screen items-center justify-center bg-background">
                <p>Could not load group settings.</p>
            </div>
        );
    }
    ...
    return (
        <div className="container mx-auto py-8">
            <Card>
                <CardHeader>
                    <CardTitle>Group Post Settings: {groupInfo.name}</CardTitle>
                    <CardDescription>Control who can post, what they can post, and how often in this specific group.</CardDescription>
                </CardHeader>
                <CardContent>
