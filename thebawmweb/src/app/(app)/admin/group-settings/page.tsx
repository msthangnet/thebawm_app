...
import { UserProfile, GroupCreationPermissions, DefaultGroupCreationPermissions } from "@/lib/types";
...
export default function GroupSettingsPage() {
    ...
    const fetchPermissions = useCallback(async () => {
        setLoadingData(true);
        try {
            const settingsRef = doc(db, "app_settings", "groupCreationPermissions");
            const docSnap = await getDoc(settingsRef);
            if (docSnap.exists()) {
                const perms = docSnap.data() as GroupCreationPermissions;
                setPermissions(perms);
                await fetchAllowedUsers(perms.allowedUserIds);
            } else {
                // If it doesn't exist, create it with default values
                await setDoc(settingsRef, DefaultGroupCreationPermissions);
                setPermissions(DefaultGroupCreationPermissions);
                setAllowedUsers([]);
            }
        } catch (error) {
    ...
    const handleAddUser = async (userId: string) => {
        ...
        try {
            ...
            toast({ title: "User Added", description: "Permission to create groups has been granted." });
    ...
    const handleRemoveUser = async (userId: string) => {
        setSaving(true);
         try {
            ...
            toast({ title: "User Removed", description: "Permission to create groups has been revoked." });
            await fetchPermissions(); // Refetch all data
        } catch (error) {
    ...
    return (
        <div className="container mx-auto py-8">
            <Card>
                <CardHeader>
                    <CardTitle>Group Creation Settings</CardTitle>
                    <CardDescription>Grant specific users the permission to create new Groups.</CardDescription>
                </CardHeader>
                <CardContent>
