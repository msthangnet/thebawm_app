...
import { UserProfile, BookCreationPermissions, DefaultBookCreationPermissions } from "@/lib/types";
...
export default function BookSettingsPage() {
    ...
    const fetchPermissions = useCallback(async () => {
        setLoadingData(true);
        try {
            const settingsRef = doc(db, "app_settings", "bookCreationPermissions");
            const docSnap = await getDoc(settingsRef);
            if (docSnap.exists()) {
                const perms = docSnap.data() as BookCreationPermissions;
                setPermissions(perms);
                await fetchAllowedUsers(perms.allowedUserIds);
            } else {
                await setDoc(settingsRef, DefaultBookCreationPermissions);
    ...
    const handleAddUser = async (userId: string) => {
        ...
        try {
            ...
            toast({ title: "User Added", description: "Permission to create books has been granted." });
    ...
    const handleRemoveUser = async (userId: string) => {
        setSaving(true);
         try {
            ...
            toast({ title: "User Removed", description: "Permission to create books has been revoked." });
            await fetchPermissions(); // Refetch all data
        } catch (error) {
    ...
    return (
        <div className="container mx-auto py-8">
            <Card>
                <CardHeader>
                    <CardTitle>Book Creation Settings</CardTitle>
                    <CardDescription>Grant specific users the permission to create new Books.</CardDescription>
                </CardHeader>
                <CardContent>
