...
import { UserProfile, LyricsCreationPermissions, DefaultLyricsCreationPermissions } from "@/lib/types";
...
export default function LyricsSettingsPage() {
    ...
    const fetchPermissions = useCallback(async () => {
        setLoadingData(true);
        try {
            const settingsRef = doc(db, "app_settings", "lyricsCreationPermissions");
            const docSnap = await getDoc(settingsRef);
            if (docSnap.exists()) {
                const perms = docSnap.data() as LyricsCreationPermissions;
                setPermissions(perms);
                await fetchAllowedUsers(perms.allowedUserIds);
            } else {
                await setDoc(settingsRef, DefaultLyricsCreationPermissions);
    ...
    const handleAddUser = async (userId: string) => {
        ...
        try {
            ...
            toast({ title: "User Added", description: "Permission to create lyrics has been granted." });
    ...
    const handleRemoveUser = async (userId: string) => {
        setSaving(true);
         try {
            ...
            toast({ title: "User Removed", description: "Permission to create lyrics has been revoked." });
            await fetchPermissions(); // Refetch all data
        } catch (error) {
    ...
    return (
        <div className="container mx-auto py-8">
            <Card>
                <CardHeader>
                    <CardTitle>Lyrics Creation Settings</CardTitle>
                    <CardDescription>Grant specific users the permission to create new Lyrics.</CardDescription>
                </CardHeader>
                <CardContent>
