...
import { UserProfile, VideoCreationPermissions, DefaultVideoCreationPermissions } from "@/lib/types";
...
export default function VideoSettingsPage() {
    ...
    const fetchPermissions = useCallback(async () => {
        setLoadingData(true);
        try {
            const settingsRef = doc(db, "app_settings", "videoCreationPermissions");
            const docSnap = await getDoc(settingsRef);
            if (docSnap.exists()) {
                const perms = docSnap.data() as VideoCreationPermissions;
                setPermissions(perms);
                await fetchAllowedUsers(perms.allowedUserIds);
            } else {
                await setDoc(settingsRef, DefaultVideoCreationPermissions);
    ...
    const handleAddUser = async (userId: string) => {
        ...
        try {
            ...
            toast({ title: "User Added", description: "Permission to publish videos has been granted." });
    ...
    const handleRemoveUser = async (userId: string) => {
        setSaving(true);
         try {
            ...
            toast({ title: "User Removed", description: "Permission to publish videos has been revoked." });
            await fetchPermissions(); // Refetch all data
        } catch (error) {
    ...
    return (
        <div className="container mx-auto py-8">
            <Card>
                <CardHeader>
                    <CardTitle>Video Publishing Settings</CardTitle>
                    <CardDescription>Grant specific users the permission to publish new Videos.</CardDescription>
                </CardHeader>
                <CardContent>
