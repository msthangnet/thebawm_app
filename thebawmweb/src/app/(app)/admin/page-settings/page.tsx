...
import { collection, doc, getDoc, updateDoc, arrayUnion, arrayRemove, query, where, getDocs, limit } from "firebase/firestore";
import { UserProfile, PageCreationPermissions, DefaultPageCreationPermissions } from "@/lib/types";
...
export default function PageSettingsPage() {
    ...
    const fetchAllowedUsers = useCallback(async (userIds: string[]) => {
        ...
        try {
            ...
            // Firestore 'in' query is limited to 30 elements. Batch if needed for more users.
    ...
    const fetchPermissions = useCallback(async () => {
        setLoadingData(true);
        try {
            const settingsRef = doc(db, "app_settings", "pageCreationPermissions");
            const docSnap = await getDoc(settingsRef);
            if (docSnap.exists()) {
                const perms = docSnap.data() as PageCreationPermissions;
                setPermissions(perms);
                await fetchAllowedUsers(perms.allowedUserIds);
            } else {
                setPermissions(DefaultPageCreationPermissions);
                setAllowedUsers([]);
            }
        } catch (error) {
    ...
    const handleAddUser = async (userId: string) => {
        ...
        try {
            ...
            toast({ title: "User Added", description: "Permission to create pages has been granted." });
    ...
    const handleRemoveUser = async (userId: string) => {
        setSaving(true);
         try {
            ...
            toast({ title: "User Removed", description: "Permission to create pages has been revoked." });
            await fetchPermissions(); // Refetch all data
        } catch (error) {
    ...
    return (
        <div className="container mx-auto py-8">
            <Card>
                <CardHeader>
                    <CardTitle>Page Creation Settings</CardTitle>
                    <CardDescription>Grant specific users the permission to create new Pages.</CardDescription>
                </CardHeader>
                <CardContent>
