...
type EventAnnouncementsTabProps = {
    event: EventInfo;
    initialPosts: Post[];
...
export function EventAnnouncementsTab({ event, initialPosts, permissions, canPost }: EventAnnouncementsTabProps) {
    const [posts, setPosts] = useState<Post[]>(initialPosts);
    
    useEffect(() => {
        const postsQuery = query(
            collection(db, 'eventAnnouncementPosts'),
        ...
        const unsubscribe = onSnapshot(postsQuery, async (snapshot) => {
            ...
            setPosts(newPosts.filter((p): p is Post => p !== null));
             ...
             console.error("Error fetching realtime announcements:", error);
            // Fallback for missing index
            const fallbackQuery = query(collection(db, 'eventAnnouncementPosts'), where('eventId', '==', event.id));
    ...
    return (
        <div className="max-w-2xl mx-auto">
            {canPost && <CreateEventPost event={event} postType="event_announcement" />}
            ...
            {posts.length > 0 ? (
                ...
                <div className="text-center py-16 text-muted-foreground">
                    <p>No announcements yet. {canPost ? "Be the first to create one!" : ""}</p>
                </div>
            )}
        </div>
    );
