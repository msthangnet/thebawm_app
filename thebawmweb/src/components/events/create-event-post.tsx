...
import { EventInfo, PostPermissions, DefaultPostPermissions } from "@/lib/types";

type CreateEventPostProps = {
  event: EventInfo;
  postType: 'event' | 'event_announcement';
};


async function getPostPermissions(eventId?: string): Promise<PostPermissions> {
    if (eventId) {
        const settingsRef = doc(db, "events", eventId, "settings", "postPermissions");
...
export function CreateEventPost({ event, postType }: CreateEventPostProps) {
  ...
  const isSpecialPoster = Array.isArray(event.posters) && event.posters.includes(user?.uid || '');
  const isParticipant = !isOwner && !isSiteAdmin && !isSpecialPoster;

   const fetchPermissions = useCallback(async () => {
    if (!user) return;
    setCheckingPermissions(true);
    try {
        const perms = await getPostPermissions(event.id);
  ...
  const fetchPostCount = useCallback(async () => {
    ...
    try {
        ...
        setPostsMade(querySnapshot.size);
    } catch (error) {
        console.error("Error fetching post count:", error);
    } finally {
        setCheckingPosts(false);
    }
  }, [user, event.id, isParticipant]);
  ...
  const handlePost = async () => {
    ...
    setLoading(true);
    const collectionName = postType === 'event' ? 'eventsPost' : 'eventAnnouncementPosts';
    const sourceText = `Post on event '${event.name}'`;
    ...
    try {
      ...
      if (mediaFiles.length > 0) {
        mediaUrls = await Promise.all(
            mediaFiles.map(async (file) => {
                const storageRef = ref(storage, `${collectionName}/${event.id}/${user.uid}/${Date.now()}_${file.name}`);
      ...
      toast({
        ...
        description: scheduleDate ? `Your post will be published on ${format(scheduleDate, 'PPp')}.` : `Your post has been shared in the ${postType === 'event' ? 'event' : 'announcements'}.`,
      });
      fetchPostCount();
      router.refresh();
  ...
  const limit = event.participantPostLimit || 5;
  const limitExceeded = isParticipant && postsMade >= limit;
  ...
  const canSchedule = postType === 'event_announcement' ? (isOwner || isSiteAdmin) : permissions.canSchedulePost.includes(userType);


  const isPostDisabled = (!postText.trim() && mediaFiles.length === 0) || loading || limitExceeded;
  
  let placeholderText = postType === 'event' ? `Write something for the event...` : 'Write an announcement...';
  if (limitExceeded) {
      placeholderText = `You have reached your post limit of ${limit} for this event.`;
  }
      ...
      <CardContent className="p-2 sm:p-4">
        <div className="w-full">
            ...
            <div className="mt-4 flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                     {isParticipant && !checkingPosts && <p className="text-xs text-muted-foreground">{postsMade}/{limit} posts</p>}
                    
                    {(isOwner || isSiteAdmin) && (
                        <Button variant="ghost" size="icon" aria-label="Post Settings" onClick={() => router.push(`/events/${event.eventId}/admin-post-settings`)}>
