...
import { PostPermissions, DefaultPostPermissions, PageInfo } from "@/lib/types";
import { Popover, PopoverContent, PopoverTrigger } from "../ui/popover";
import { Calendar } from "../ui/calendar";
import { format } from "date-fns";

type CreatePagePostProps = {
  pageId: string;
  pageInfo: PageInfo | null;
};

async function getPostPermissions(pageId?: string): Promise<PostPermissions> {
    if (pageId) {
        const settingsRef = doc(db, "pages", pageId, "settings", "postPermissions");
...
export function CreatePagePost({ pageId, pageInfo }: CreatePagePostProps) {
  ...
  const fetchPermissions = useCallback(async () => {
    ...
    try {
        const perms = await getPostPermissions(pageId);
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      if (mediaFiles.length > 0) {
          mediaUrls = await Promise.all(
              mediaFiles.map(async (file) => {
                  const storageRef = ref(storage, `pagesPost/${pageId}/${user.uid}/${Date.now()}_${file.name}`);
      ...
      await addDoc(collection(db, "pagesPost"), {
        authorId: user.uid,
        pageId: pageId,
        text: postText,
        mediaUrls: mediaUrls,
        ...
        source: `Post on Page '${pageInfo?.name}'`,
      });
  ...
  const placeholderText = `Write something on this page...`;
        ...
        {checkingPermissions ? (
            ...
            <div className="w-full">
                ...
                <div className="mt-4 flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-2">
                        {(isOwner || isSiteAdmin) && pageInfo && (
                            <Button variant="ghost" size="icon" aria-label="Post Settings" onClick={() => router.push(`/pages/${pageInfo.pageId}/admin-post-settings`)}>
