...
import { Image as ImageIcon, Video, Send, Loader2, X, Settings, CalendarClock } from "lucide-react";
import { useState, useRef, useEffect, useCallback } from "react";
import { collection, addDoc, serverTimestamp, getDoc, doc, Timestamp, query, where, getDocs, orderBy } from "firebase/firestore";
import { db, storage } from "@/lib/firebase";
import { ref, uploadBytes, getDownloadURL } from "firebase/storage";
import { useToast } from "@/hooks/use-toast";
import { useRouter } from "next/navigation";
import Image from "next/image";
import { PostPermissions, DefaultPostPermissions } from "@/lib/types";
...
export function CreateUserPost() {
  ...
  const fetchPermissionsAndPosts = useCallback(async () => {
    if (!user) return;
    setCheckingPermissions(true);
    try {
        const perms = await getPostPermissions();
        setPermissions(perms);

        const twentyFourHoursAgo = Timestamp.fromMillis(Date.now() - 24 * 60 * 60 * 1000);
        const postsQuery = query(
            collection(db, "usersPost"),
            where("authorId", "==", user.uid),
            where("createdAt", ">=", twentyFourHoursAgo),
            orderBy("createdAt", "desc")
        );
        const querySnapshot = await getDocs(postsQuery);
        setPostsToday(querySnapshot.size);
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      if (mediaFiles.length > 0) {
          mediaUrls = await Promise.all(
              mediaFiles.map(async (file) => {
                  const storageRef = ref(storage, `usersPost/${user.uid}/${Date.now()}_${file.name}`);
      ...
      await addDoc(collection(db, "usersPost"), {
        authorId: user.uid,
        text: postText,
        mediaUrls: mediaUrls,
        mediaType: mediaType,
        ...
        source: `Post on user timeline '${userProfile.displayName}'`,
      });
      ...
      toast({
        ...
        description: scheduleDate ? `Your post will be published on ${format(scheduleDate, 'PPp')}.` : "Your post has been shared.",
      });
      fetchPermissionsAndPosts(); // Refresh post count
      router.refresh();
  ...
  const canPostFeed = permissions.canPost.includes(userType);
  const postLimit = permissions.dailyPostLimit[userType] || 0;
  const imageUploadLimit = permissions.imageUploadLimit[userType] || 1;
  const limitExceeded = postsToday >= postLimit;
  ...
  const placeholderText = limitExceeded 
    ? `You have reached your daily post limit of ${postLimit}.`
    : `What's on your mind, ${userProfile?.displayName || userProfile?.username}?`;
            ...
            <div className="w-full">
                ...
                <div className="mt-4 flex justify-between items-center">
                    <div className="flex gap-1 items-center">
                        ...
                        <input type="file" accept="image/*" multiple={imageUploadLimit > 1} ref={imageInputRef} onChange={handleFileChange} className="hidden" disabled={!canUploadImage || mediaType === 'video'}/>
                    ...
                    <div className="flex items-center gap-2">
                         {isAdmin && (
                            <Button variant="ghost" size="icon" aria-label="Post Settings" onClick={() => router.push('/admin/settings')}>
                 ...
                 {canUploadImage && imageUploadLimit > 1 && (
                     <p className="text-xs text-muted-foreground text-left mt-2">
                        You can upload up to {imageUploadLimit} images.
                    </p>
                )}
                {postLimit < 999 && (
                    <p className="text-xs text-muted-foreground text-right mt-1">
                        {postsToday}/{postLimit} posts today
                    </p>
                )}
            </div>
            </div>
