...
import { QuizInfo, PostPermissions, DefaultPostPermissions } from "@/lib/types";

type CreateQuizPostProps = {
  quiz: QuizInfo;
  postType: 'quiz' | 'quiz_announcement';
};


async function getPostPermissions(quizId?: string): Promise<PostPermissions> {
    if (quizId) {
        const settingsRef = doc(db, "quizzes", quizId, "settings", "postPermissions");
...
export function CreateQuizPost({ quiz, postType }: CreateQuizPostProps) {
  ...
  const isSiteAdmin = userProfile?.userType === 'Admin';
   ...
   const fetchPermissions = useCallback(async () => {
    ...
    try {
        const perms = await getPostPermissions(quiz.id);
  ...
  const handlePost = async () => {
    ...
    setLoading(true);
    const collectionName = postType === 'quiz' ? 'quizzesPost' : 'quizAnnouncementPosts';
    const sourceText = `Post on quiz '${quiz.name}'`;
    ...
    try {
      ...
      if (mediaFiles.length > 0) {
        mediaUrls = await Promise.all(
            mediaFiles.map(async (file) => {
                const storageRef = ref(storage, `${collectionName}/${quiz.id}/${user.uid}/${Date.now()}_${file.name}`);
      ...
      toast({
        ...
        description: scheduleDate ? `Your post will be published on ${format(scheduleDate, 'PPp')}.` : `Your post has been shared in the ${postType === 'quiz' ? 'quiz' : 'announcements'}.`,
      });
      router.refresh();
  ...
  let placeholderText = postType === 'quiz' ? `Write something for the quiz...` : 'Write an announcement...';
      ...
      <CardContent className="p-2 sm:p-4">
        <div className="w-full">
            ...
            <div className="mt-4 flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                    {(isOwner || isSiteAdmin) && (
                        <Button variant="ghost" size="icon" aria-label="Post Settings" onClick={() => router.push(`/quizzes/${quiz.quizId}/admin-post-settings`)}>
